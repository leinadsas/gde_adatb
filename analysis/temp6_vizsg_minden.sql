SELECT 
    ...
    ) AS arany_95fok_120fok_között_szazalek,
    
    SUM(CASE 
            WHEN c.temp_6 IS NOT NULL AND c.temp_6 > 120
            THEN 1 
            ELSE 0 
        END) AS meresek_120fok_felett,
    ...
    ROUND(
        100.0 * SUM(CASE 
                    ...
                    END) / 
        NULLIF(COUNT(c.temp_6), 0), 2
    ) AS arany_120fok_felett_szazalek,
    
    
    ROUND(
        100.0 * SUM(CASE 
                        WHEN c.temp_6 IS NOT NULL AND c.temp_6 > 95
                        THEN 1 
                        ELSE 0 
                    END) / 
        NULLIF(COUNT(c.temp_6), 0), 2
    ) AS arany_95fok_felett
